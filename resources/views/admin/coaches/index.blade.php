            ...
            <table class="table table-bordered table-striped {{ count($data) > 0 ? 'datatable' : '' }} @can('coach_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        ...
                        <th>@lang('quickadmin.coach.fields.id')</th>
                        <th>@lang('quickadmin.coach.fields.name')</th>
                        <th>@lang('quickadmin.coach.fields.photo')</th>
                        <th>@lang('quickadmin.coach.fields.description')</th>
                        <th>@lang('quickadmin.coach.fields.sport-section')</th>
                        <th>@lang('quickadmin.coach.fields.work')</th>
                    ...
                    @if (count($data) > 0)
                        @foreach ($data as $coach)
                            <tr data-entry-id="{{ $coach->id }}">
                                ...
                                <td field-key='id'>{{ $coach->id }}</td>
                                <td field-key='name'>{{ $coach->name }}</td>
                                <td field-key='photo'>
                                    @if($coach->photo)
                                        <img class="photo" src="{{ asset($coach->getImageAdminPanel()) }}">
                                    @endif
                                </td>
                                <td field-key='description'>{!! $coach->description !!}</td>
                                <td>{{ $coach->getSectionTitle() }}</td>
                                <td field-key='work'>{{ $coach->work }}</td>
                                @if( request('show_deleted') == 1 )
                                <td>
                                    @can('coach_delete')
                                                                        {!! Form::open(array(
