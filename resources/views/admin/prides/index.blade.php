...
@section('content')
    ...
    @can('pride_delete')
        ...
        <ul class="list-inline">
            <li><a href="{{ route('admin.prides.index') }}"
                   style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a>
            </li>
            |
            <li><a href="{{ route('admin.prides.index') }}?show_deleted=1"
    ...
    <div class="panel panel-default">
                    ...
                    @foreach ($data as $pride)
                        <tr data-entry-id="{{ $pride->id }}">
                            ...
                            <td field-key='photo'>
                                @if($pride->photo)
                                    <img class="photo"
                                         src="{{ asset($pride->getImageAdminPanel()) }}">
                                @endif
                            </td>
        ...
        'route' => ['admin.prides.perma_del', $pride->id])) !!}
                            ...
                            @else
                                <td>                                    @can('pride_edit')
                                        <a href="{{ route('admin.prides.edit',[$pride->id]) }}"
